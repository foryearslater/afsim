                                         ...
                                         const WsfWeapon::FireOptions&  aSettings)
{
   ...
   // - the firing weapon
   auto eventPtr = ut::make_unique<FireUpdateEvent>();
   eventPtr->mPlatformIndex = GetPlatform()->GetIndex();
   eventPtr->mWeaponPtr = this;
   ...
   if (result.mSuccess)
   {
      ...
      eventPtr->mFireTimeLeft = mFireDuration;
      eventPtr->SetTime(aSimTime + mFireIntegrationInterval);
