...
$pgm_name = "evt_extract_sample3.pl";
...
###############################################################
# ADD USER-SPECIFIED DATA STRUCTURES TO COLLECT DATA          #
###############################################################

# Initialized hash arrays to save first track ranges accessed
#  by <Target ID> and <Sensor ID>
%first_trk_rngs = ();
...
while (1) {
    ...
    ########################################################
    # Collect first detection ranges for desired target IDs
    #  and the associated sensors:
    if (not defined $first_trk_rngs{$tgt_id}{$sensor_id}) {
	$first_trk_rngs{$tgt_id}{$sensor_id} = $range_nmi;
    }
    ########################################################
...
##########################################################
# Add code to output first track ranges for each
#  target, for each sensor
##########################################################


$trkrngfilename = $rootfilename . "_TRK_RNG.csv"; # Add "_TRK_RNG.csv" to name

if (not open(RNGFH, '>'.$trkrngfilename)) {
    print "\nOutput file \"$trkrngfilename\" Could not be opened";
    print "\nPress ENTER key to Abort: ";
    <STDIN>;
    die("\nTerminating execution ... \n");
}

print "\nWriting First Target-in-Track Ranges to file: $trkrngfilename";

# Write header line for detection range file:
$outline = "Target ID,Sensor ID,1st Track Range (N Mi)";
# Write processed data to detection range file:
&writeln(\*RNGFH, $trkrngfilename, $outline);

# Get sorted list of Target IDs:
@tgts = sort(keys %first_trk_rngs);

foreach $tgt (@tgts) {
    # Get sorted list of sensor IDs:
    @sensors = sort(keys %{$first_trk_rngs{$tgt}});

    foreach $sensor (@sensors) {
	# Extract first detection range:
	$rng_nmi = $first_trk_rngs{$tgt}{$sensor};

	$outline = "$tgt,$sensor,$rng_nmi";
	# Write processed data to file:
	&writeln(\*RNGFH, $outfilename, $outline);
    }
}
...
=head1 NAME - evt_extract_sample3.pl

=head2 SYNOPSIS

evt_extract_sample3.pl - Filter and parse "LOCAL_TRACK_INITIATED"
...
This script uses the Perl script "evt_extract_sample2.pl" (which was
derived from "evt_extract_sample1.pl", which was derived from
"evt_extract_BASE.pl") as a starting-point for added additional
functionality for filtering and processing AFSIM output Event
files. See the AFSIM distribution "tools" folder for this and other
useful Perl scripts.
...
The "evt_extract_sample2.pl" Perl script provides the following
additional features:
...
Range in Nautical Miles from Event file's default Range in Meters. (A
...
Add a "hash array" ("%first_trk_rngs" -- also known as an "associative
array") to store the first detection range for each Target Id by each
Sensor ID. (NOTE: An "array" stores and accesses its data by using
numerical indices, while a "hash array" stores and accesses its data
by using text keywords.)

=item *

Add code to store only the first track range for each Target ID, for
each Sensor ID, for those Sensor Types of "ew_radar" and "acq_radar".

=item *

After all Event file data has been read, Open a new file for output
called "<root_file_name>_TRK_RNG.csv", extract the Target and Sensor
ID keywords from the first-track-range hash array, and output the
table of Target ID, Sensor ID, and First Track Range data. (NOTE:
While this code is specific to this script, it also illustrates the
general technique for extracting keywords and accessing data from a
"hash array".
...
enter:
 ...
 In a Windows environment:
   perldoc -F evt_extract_sample3.pl > evt_extract_sample3_doc.txt

 In a Linux environment:
   perldoc -otext -F evt_extract_sample3_doc.pl > evt_extract_sample3_doc.txt
...
To convert POD documentation to HTML format, enter:
 ...
 pod2html --infile=evt_extract_sample3.pl --outfile=evt_extract_sample3_doc.html
