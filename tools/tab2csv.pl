...
while (<INFH>) {
    ...
    # Substitute each area delimited by tab with a comma:
    $outline =~ s/\t/\,/g;
...
=head1 NAME - tab2csv.pl

=head2 SYNOPSIS

tab2csv.pl - Convert any Tab characters in the input file to commas.
...
items in tabular format with data items on each row separated by Tab
characters into comma-separated-variable (CSV) format files that can
be processed with Microsoft "Excel" (Windows operating system
environment) or "oocalc" (Linux operating system environment). (See
the similar "spc2csv.pl" Perl script that generates CSV files using
Whitespace character groups instead of Tab characters as data item
separators.)
...
enter:
 ...
 In a Windows environment:
   perldoc -F tab2csv.pl > tab2csv_doc.txt

 In a Linux environment:
   perldoc -otext -F tab2csv.pl > tab2csv_doc.txt
...
To convert POD documentation to HTML format, enter:
 ...
 pod2html --infile=tab2csv.pl --outfile=tab2csv_doc.html
